window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = [
      ...
      ".dgh_dashboard{display:flex;flex-direction:column;gap:14px;width:100%;color:var(--dsw-alias-label-primary);padding:4px}",
      ".dgh_head{display:flex;align-items:center;gap:14px}",
      ".dgh_avatar{width:56px;height:56px;border-radius:999px;border:1px solid var(--dsw-alias-border-l2)}",
      ".dgh_name{margin:0;font-size:18px;line-height:26px}",
      ".dgh_bio{color:var(--dsw-alias-label-secondary);font-size:13px;line-height:20px;margin:0}",
      ".dgh_heatmap{display:flex;gap:3px;overflow-x:auto;padding:4px 0}",
      ".dgh_week{display:flex;flex-direction:column;gap:3px}",
      ".dgh_day{width:11px;height:11px;border-radius:2px;display:inline-block}",
      ".dgh_grid{display:grid;grid-template-columns:minmax(0,1fr);gap:10px}",
      ".dgh_repo{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-3);border-radius:10px;padding:12px;display:flex;flex-direction:column;gap:6px}",
      ".dgh_repoTitle{display:flex;align-items:center;justify-content:space-between;gap:8px;min-width:0}",
      ".dgh_repoName{color:var(--dsw-alias-label-primary);font-size:14px;font-weight:600;text-decoration:none}",
      ".dgh_repoName:hover{text-decoration:underline}",
      ".dgh_meta{color:var(--dsw-alias-label-tertiary);font-size:12px;line-height:18px;margin:0}",
      ".dgh_row{display:flex;flex-wrap:wrap;gap:8px;align-items:center}",
      ".dgh_button_sm{height:28px;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-1);color:var(--dsw-alias-label-primary);border-radius:6px;padding:0 10px;font:inherit;font-size:12px;cursor:pointer}",
      ".dgh_button_sm:hover{background:var(--dsw-alias-interactive-bg-hover)}",
      ".dgh_button_danger{color:var(--dsw-alias-state-error-primary);border-color:var(--dsw-alias-state-error-primary)}",
      ".dgh_branch{display:flex;align-items:center;justify-content:space-between;gap:10px;border-bottom:1px solid var(--dsw-alias-border-l2);padding:7px 0}",
      ".dgh_branchName{font-family:var(--ds-font-family-code);font-size:12px;color:var(--dsw-alias-label-primary)}",
      ".dgh_current{color:var(--dsw-alias-state-business-primary);font-size:11px}",
      ".dgh_notification{border-bottom:1px solid var(--dsw-alias-border-l2);padding:8px 0;display:flex;flex-direction:column;gap:4px}",
      ".dgh_scroll{max-height:320px;overflow:auto;display:flex;flex-direction:column}",
    ].join("\n")
    ...
    function makeGhApi(ctx) {
      ...
      return {
        ...
        profile: function () { return call("gh/profile", {}) },
        repositories: function () { return call("gh/repositories", {}) },
        inbox: function () { return call("gh/inbox", {}) },
        branches: function (cwd) { return call("gh/branches", { cwd: cwd || "" }) },
        repoBranches: function (repo) { return call("gh/repoBranches", { repo: repo }) }
      }
    }
    ...
    function heatmapView(heatmap) {
      if (!heatmap || !heatmap.weeks || heatmap.weeks.length === 0) {
        return create("p", { className: "dgh_sub" }, "暂无贡献热力图数据")
      }
      return create("div", { className: "dgh_heatmap" },
        heatmap.weeks.map(function (week, index) {
          return create("div", { className: "dgh_week", key: week.days && week.days[0] ? week.days[0].date : "week-" + index },
    ...
    function GhDashboardView(props) {
      var gh = props.gh
      var cwd = props.cwd || ""
      ...
      var inboxState = React.useState(null)
      var inbox = inboxState[0]
      var setInbox = inboxState[1]
      var localBranchesState = React.useState(null)
      var localBranches = localBranchesState[0]
      var setLocalBranches = localBranchesState[1]
      ...
      function loadAll() {
        setBusy(true)
        Promise.all([
          gh.profile(), gh.repositories(), gh.inbox(), gh.branches(cwd)
        ]).then(function (results) {
          setProfile(results[0])
          setRepos(results[1])
          setInbox(results[2])
          setLocalBranches(results[3])
          setBusy(false)
        }, function () {
          setBusy(false)
        })
      ...
      function runAction(title, promise) {
        setBusy(true)
        setAction({ title: title, result: null })
        promise.then(function (value) {
          setAction({ title: title, result: value })
          setBusy(false)
        }, function (reason) {
          setAction({ title: title, result: { ok: false, error: reason && reason.message ? reason.message : String(reason) } })
          setBusy(false)
        })
      }

      function actionResultText(value) {
        if (!value) return ""
        if (value.error) return value.error
        if (value.parsed !== null && value.parsed !== undefined) return JSON.stringify(value.parsed, null, 2)
        if (value.stdout) return value.stdout
        if (value.stderr) return value.stderr
        if (value.branches) return JSON.stringify(value.branches, null, 2)
        return JSON.stringify(value, null, 2)
      }

      function repoDelete(repo) {
        var yes = typeof window !== "undefined" && typeof window.confirm === "function"
          ? window.confirm("确定删除 " + repo + " 吗？此操作不可撤销。")
          : false
        if (!yes) return
        runAction("删除仓库 " + repo, gh.execute("gh_repo_delete", { repo: repo, confirm: true }))
      }

      return create("div", { className: "dgh_dashboard" },
        create("div", { className: "dgh_card" },
          create("div", { className: "dgh_head" },
            profile && profile.profile && profile.profile.avatarUrl
              ? create("img", { className: "dgh_avatar", src: profile.profile.avatarUrl, alt: "" })
              : null,
            create("div", null,
              create("h3", { className: "dgh_name" }, profile && profile.profile ? profile.profile.name || profile.profile.login : "GitHub"),
              create("p", { className: "dgh_bio" }, profile && profile.profile ? profile.profile.bio || profile.profile.url : "加载中…"),
              create("p", { className: "dgh_sub" }, profile && profile.heatmap ? "过去一年 " + profile.heatmap.totalContributions + " contributions" : ""))),
          create("h3", null, "贡献热力图"),
          heatmapView(profile && profile.heatmap),
          create("div", { className: "dgh_row" },
            create("button", { className: "dgh_button_sm", disabled: busy, onClick: loadAll }, "刷新全部"))),
        create("div", { className: "dgh_card" },
          create("h3", null, "仓库列表"),
          repos && repos.repositories && repos.repositories.length > 0
            ? create("div", { className: "dgh_grid" },
                repos.repositories.map(function (repo) {
                  return create("div", { className: "dgh_repo", key: repo.nameWithOwner },
                    create("div", { className: "dgh_repoTitle" },
                      create("a", { className: "dgh_repoName", href: repo.url, target: "_blank", rel: "noreferrer" }, repo.nameWithOwner),
                      create("span", { className: "dgh_sub" }, repo.visibility)),
                    repo.description ? create("p", { className: "dgh_meta" }, repo.description) : null,
                    create("p", { className: "dgh_meta" }, "★ " + repo.stars + " · 默认分支 " + (repo.defaultBranch || "?") + " · " + repo.updatedAt),
                    create("div", { className: "dgh_row" },
                      create("button", { className: "dgh_button_sm", onClick: function () { runAction(repo.nameWithOwner + " Issues", gh.execute("gh_issue_list", { repo: repo.nameWithOwner })) } }, "Issues"),
                      create("button", { className: "dgh_button_sm", onClick: function () { runAction(repo.nameWithOwner + " PRs", gh.execute("gh_pr_list", { repo: repo.nameWithOwner })) } }, "PRs"),
                      create("button", { className: "dgh_button_sm", onClick: function () { runAction(repo.nameWithOwner + " 分支", gh.repoBranches(repo.nameWithOwner)) } }, "分支"),
                      create("button", { className: "dgh_button_sm dgh_button_danger", onClick: function () { repoDelete(repo.nameWithOwner) } }, "删除")))
                }))
            : create("p", { className: "dgh_sub" }, busy ? "加载仓库中…" : (repos && repos.error ? repos.error : "暂无仓库"))),
        create("div", { className: "dgh_card" },
          create("h3", null, inbox && inbox.notifications ? "收件箱（显示前 30 / 共 " + inbox.notifications.length + "）" : "收件箱"),
          inbox && inbox.notifications && inbox.notifications.length > 0
            ? create("div", { className: "dgh_scroll" }, inbox.notifications.slice(0, 30).map(function (item) {
                return create("div", { className: "dgh_notification", key: item.id },
                  create("div", { className: "dgh_repoTitle" },
                    create("a", { className: "dgh_repoName", href: item.url || "#", target: "_blank", rel: "noreferrer" }, item.title || item.type),
                    create("span", { className: "dgh_sub" }, item.reason)),
                  create("p", { className: "dgh_meta" }, item.repository + " · " + item.updatedAt),
                  create("button", { className: "dgh_button_sm", onClick: function () { runAction("标记已读 " + item.title, gh.execute("gh_api", { path: "notifications/threads/" + item.id, method: "PATCH", confirm: true })) } }, "标记已读"))
              }))
            : create("p", { className: "dgh_sub" }, inbox && inbox.error ? inbox.error : (busy ? "加载收件箱中…" : "暂无未读通知"))),
        create("div", { className: "dgh_card" },
          create("h3", null, "Git 分支"),
          cwd ? create("p", { className: "dgh_sub" }, cwd) : create("p", { className: "dgh_sub" }, "当前会话没有绑定工作区"),
          localBranches && localBranches.branches && localBranches.branches.length > 0
            ? create("div", null, localBranches.branches.map(function (branch) {
                return create("div", { className: "dgh_branch", key: branch.name },
                  create("div", null,
                    create("span", { className: "dgh_branchName" }, branch.name),
                    branch.current ? create("span", { className: "dgh_current" }, " ●") : null,
                    branch.upstream ? create("span", { className: "dgh_sub" }, " → " + branch.upstream + (branch.track ? " (" + branch.track + ")" : "")) : null),
                  create("span", { className: "dgh_meta" }, (branch.subject || "") + " · " + branch.commit))
              }))
            : create("p", { className: "dgh_sub" }, localBranches && localBranches.error ? localBranches.error : "暂无分支数据"),
          create("div", { className: "dgh_row" },
            create("button", { className: "dgh_button_sm", onClick: function () { gh.branches(cwd).then(setLocalBranches) } }, "刷新分支"))),
        action ? create("div", { className: "dgh_card" },
          create("h3", null, action.title),
          create("pre", { className: "dgh_result" }, actionResultText(action.result))) : null)
    }
      ...
      ctx.slots.inject("conversation.view", function () {
        return ctx.slots.register({
          ...
          inject: function (sessionId) {
            var snapshot = ctx.sessions && ctx.sessions.list ? ctx.sessions.list.getSnapshot() : { byId: {} }
            var row = snapshot.byId ? snapshot.byId[sessionId] : undefined
            return { gh: makeGhApi(ctx), cwd: row && row.cwd ? row.cwd : "" }
          }
        }, GhDashboardView)
      })
    }
